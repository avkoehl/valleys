...
def prepare_xsection(xs_points):
    ...
    duplicted = temp.duplicated(
        subset=["elevation", "slope", "curvature", "hand", "hillslopes", "strm_val"]
    )
...
def find_xs_break_points(df, peak_threshold=0.002, slope_threshold=20):
    ...
    pos_bp = find_half_profile_break_point(
        df.loc[df["alpha"] >= 0], peak_ids, slope_threshold
    )
    neg_bp = find_half_profile_break_point(
        df.loc[df["alpha"] <= 0], peak_ids, slope_threshold
    )
